...
class ShiroApp:
    ...
    def handle_autonomous_speak(self, text: str, speech_type: str):
        ...
        speech_type routing:
          "bubble_continuation" — follow-up bubble from a direct reply.
              These are already in memory (stored as part of full_response).
              Just queue for display as a new assistant message.
          anything else — genuine autonomous thought/proactive message.
              Queue for display. Engine already stored these in short_term_buffer.
        """
        # Both types go to results_queue for the GUI to display.
        # The sentinel None as user tag signals: no user message to show.
