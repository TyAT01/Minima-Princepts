...
def config():
    return {
        "memory": {"db_path": "./test_shiro_memory", "collection_name": "test_collection"},
        "llm": {"model": "shiro:latest", "api_type": "ollama"},
        "persona": {"sheet_path": "shiro_sheet.yaml"}
    }
...
async def test_fetch_relevant_memory_async(config):
    with patch("shiro_engine.MemoryStore") as MockMemory:
...
async def test_handle_tool_calls_async(config):
    with patch("shiro_engine.LlamaClient") as MockLLM:
...
async def test_check_and_think_async_triggered(config, mock_brain_data):
    with patch("shiro_engine.MemoryStore") as MockMemory, \
         patch("shiro_engine.LlamaClient") as MockLLM:
