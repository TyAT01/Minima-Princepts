...
class PersonaManager:
    ...
    def __init__(self, sheet_path: str = "Seed_sheet.yaml"):
    ...
    def load_persona(self):
        ...
        search_paths: List[Path] = [
            self.sheet_path,
            Path("Seed-AI/Seed_sheet.yaml"),
            Path("../Seed-AI/Seed_sheet.yaml"),
            Path("Seed_sheet.yaml"),
            # Search from script location
            Path(__file__).resolve().parent.parent / "Seed_sheet.yaml",
            Path(__file__).resolve().parent.parent.parent / "Seed-AI" / "Seed_sheet.yaml",
        ]

        # Add even more candidate folders by looking for any folder named *chroma* or *Seed*
        try:
            cwd = Path.cwd()
            for p in [cwd, cwd.parent]:
                for candidate in p.glob("**/Seed_sheet.yaml"):
        ...
        if not found_path:
            logger.error(f"Character sheet 'Seed_sheet.yaml' not found. Please ensure it exists.")
            self.system_prompt = "You are Seed, a helpful AI companion."
            return
    ...
    def _build_system_prompt(self, now: datetime = None):
        ...
        # Check for pre-defined markdown prompt (Prioritize this if present)
        if 'system_prompt_markdown' in self.persona_data:
            prompt = temporal_context + self.persona_data['system_prompt_markdown']

            # Append Response Style if present to ensure system compatibility
        ...
        prompt += "Example: [THOUGHT] User seems curious about machines. I'll recall 'Seed Sr.' and respond with mechanical enthusiasm. [/THOUGHT] Yah-hoh~ Proxy! Want to see how I tweaked Seed Sr.'s joints? \n\n"
