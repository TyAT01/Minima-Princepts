...
class PersonaManager:
    ...
    def __init__(self, sheet_path: str = "lumina_sheet.yaml"):
    ...
    def load_persona(self):
        ...
        search_paths: List[Path] = [
            self.sheet_path,
            Path("Lumina-AI/lumina_sheet.yaml"),
            Path("../Lumina-AI/lumina_sheet.yaml"),
            Path("lumina_sheet.yaml"),
            # Search from script location
            Path(__file__).resolve().parent.parent / "lumina_sheet.yaml",
            Path(__file__).resolve().parent.parent.parent / "Lumina-AI" / "lumina_sheet.yaml",
        ]

        # Add even more candidate folders by looking for any folder named *lumina*
        try:
            cwd = Path.cwd()
            for p in [cwd, cwd.parent]:
                for candidate in p.glob("**/lumina_sheet.yaml"):
        ...
        if not found_path:
            logger.error(f"Character sheet 'lumina_sheet.yaml' not found. Please ensure it exists.")
            self.system_prompt = "You are Lumina, a sassy and kind synthetic girl."
            return
    ...
    def _build_system_prompt(self, now: datetime = None):
        ...
        # 2. Fallback to old nested structure (Modified for Lumina defaults)
        char = self.persona_data.get('character', {})
        name = char.get('name', 'Lumina')
