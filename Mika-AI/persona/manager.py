...
class PersonaManager:
    ...
    def __init__(self, sheet_path: str = "mika_sheet.yaml"):
    ...
    def load_persona(self):
        ...
        search_paths: List[Path] = [
            self.sheet_path,
            Path("Mika-AI/mika_sheet.yaml"),
            Path("../Mika-AI/mika_sheet.yaml"),
            Path("mika_sheet.yaml"),
            # Search from script location
            Path(__file__).resolve().parent.parent / "mika_sheet.yaml",
            Path(__file__).resolve().parent.parent.parent / "Mika-AI" / "mika_sheet.yaml",
        ]

        # Add even more candidate folders by looking for any folder named *mika*
        try:
            cwd = Path.cwd()
            for p in [cwd, cwd.parent]:
                for candidate in p.glob("**/mika_sheet.yaml"):
        ...
        if not found_path:
            logger.error(f"Character sheet 'mika_sheet.yaml' not found. Please ensure it exists.")
            self.system_prompt = "You are Mika, a high-energy biker girl."
            return
    ...
    def _build_system_prompt(self, now: datetime = None):
        ...
        if 'persona' in self.persona_data:
            ...
            if 'system_prompt_template' in pers:
                ...
                prompt += f"You are {pers.get('name', 'Mika')}.\n\n"
        ...
        # 2. Fallback to old nested structure (Modified for Mika defaults)
        char = self.persona_data.get('character', {})
        name = char.get('name', 'Mika')
