...
class AureliaOrchestrator:
    def __init__(
        ...
        self.personaplex = personaplex or PersonaPlex()
    ...
    def _process_internal_commands(self, text: str) -> str:
        """Scans for internal commands ([CADENCE], [PERSONAPLEX]), applies them, and returns cleaned text."""
        # 1. Handle Cadence
        cadence_pattern = r"\[CADENCE:\s*(.*?)\]"
        cadence_matches = re.findall(cadence_pattern, text, re.IGNORECASE)
        ...
        # 2. Handle Personaplex
        if self.personaplex:
            changed = self.personaplex.handle_command(text)
            if changed:
        ...
        cleaned = re.sub(r"\[PERSONAPLEX:\s*(.*?)\]", "", cleaned, flags=re.IGNORECASE)
