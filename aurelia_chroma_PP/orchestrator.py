...
from llm.persona import get_pp_manager, load_persona_prompt
...
class AureliaOrchestrator:
    ...
    def _extract_and_apply_cadence_commands(self, text: str) -> str:
        ...
        # 1. Handle Personaplex tags
        pp_pattern = r"\[PERSONAPLEX:\s*(.*?)\]"
        pp_matches = re.findall(pp_pattern, text, re.IGNORECASE)
        pp_updated = False
        ...
        for match in pp_matches:
            ...
            if action == "modify" and module:
                trait = params.get("trait")
                value = params.get("value")
                if trait and value:
                    get_pp_manager().modify_module(module, {"personality_traits": {trait: {"description": value}}})
                    pp_updated = True
            elif action == "activate" and module:
                if module not in get_pp_manager().active_modules:
                    get_pp_manager().active_modules.append(module)
                    pp_updated = True

        if pp_updated:
            new_prompt = load_persona_prompt()
            self.chroma_client.set_persona_prompt(new_prompt)
            logger.info("Personaplex updated and new prompt applied to ChromaClient.")

        # 2. Handle Cadence tags
        pattern = r"\[CADENCE:\s*(.*?)\]"
        ...
        # Strip all tags from output and normalize spaces
        cleaned = re.sub(pp_pattern, "", text, flags=re.IGNORECASE)
