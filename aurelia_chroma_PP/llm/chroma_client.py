...
class ChromaClient:
    ...
    def stream_respond_to_text(self, text: str, context: str = "") -> Generator[str, None, None]:
        ...
        inputs = self._processor(text, conversations=conversation, prompt_audio=None, add_generation_prompt=True, return_tensors="pt")
    ...
    def stream_respond_to_audio(self, audio_path: str, context: str = "") -> Generator[str, None, None]:
        ...
        inputs = self._processor(None, conversations=conversation, prompt_audio=audio_tensor, add_generation_prompt=True, return_tensors="pt")
        inputs = {k: v.to(self._device) for k, v in inputs.items()}
    ...
    def generate_audio_for_fragment(self, fragment: str) -> Optional[np.ndarray]:
        ...
        try:
            ...
            inputs = self._processor(fragment, conversations=conversation, prompt_audio=None, add_generation_prompt=False, return_tensors="pt")
            inputs = {k: v.to(self._device) for k, v in inputs.items()}
    ...
    def _generate_response(self, conversation: list, prompt_audio: Optional[torch.Tensor] = None, do_sample: bool = True) -> tuple[np.ndarray | None, str | None]:
        try:
            # Extract last user text if available for prompt_text positional argument
            prompt_text = ""
            if conversation and conversation[0]:
                for msg in reversed(conversation[0]):
                    if msg["role"] == "user":
            ...
            inputs = self._processor(prompt_text, conversations=conversation, prompt_audio=prompt_audio, add_generation_prompt=True, return_tensors="pt")
            inputs = {k: v.to(self._device) for k, v in inputs.items()}
