...
class TestCadenceController(unittest.TestCase):
    ...
    def test_no_reply_when_empty(self):
        self.controller.last_spoke_ts = self.now - 20.0 # High silence pressure
        signals = StreamSignals(now=self.now, chat_messages=[])

        # In our refined controller, w_reply should be 0 if buffer is empty
        self.controller.update(signals)

        # Run multiple times to ensure we never get a REPLY
        for _ in range(50):
            intent = self.controller.maybe_emit_intent(signals)
            if intent:
                self.assertNotEqual(intent.kind, "REPLY", "Should not emit REPLY intent when message buffer is empty.")
    ...
    def test_focus_suppression(self):
        ...
        self.assertIsNone(intent, f"Intent should be suppressed by focus. Got {intent.kind if intent else 'None'}")
