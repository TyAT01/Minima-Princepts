...
class LokiApp:
    ...
    def _load_config(self, path: str) -> dict:
        try:
            if not os.path.exists(path):
                ...
                alt_path = os.path.join("Loki-AI", path)
    ...
    def process_text(self, text: Any, user_name: str = None):
        ...
        logging.info(f"--- Processing Message: '{processed_text}' ---")
            ...
            try:
                ...
                if not self.interrupt_event.is_set():
                    ...
                    if self.last_thought:
                        ...
                        logging.info(f"Loki's Thought: {self.last_thought}")
                        self.memory.store_insight(f"Thought: {self.last_thought}", source="inner_monologue")

                    self.memory.add_interaction(processed_text, full_response.strip(), user_id=user_name)
    ...
    def reflect(self, user_id: str):
        ...
        try:
            with self.processing_lock:
                logging.info(f"Loki is reflecting on recent experiences with {user_id}...")
                history = self.memory.get_history()
                if not history: return

                reflection_prompt = (
                    "You are Loki, performing deep reflection. Analyze our recent chat history and extract the following:\n"
                ...
                analysis_raw = self.llm.generate_response("You are Loki, analyzing your memories.", f"Recent History: {history}", [], context=reflection_prompt)
    ...
    def get_smart_intensity(self, user_msg: str) -> float:
        ...
        history_text = " ".join([m["content"] for m in history_list]).lower()

        hype = len([w for w in ["!","??","raid","plan","now","chaos","idiot","minion"] if w in history_text])
        chill = len([w for w in ["tired","sleep","cozy","soft","quiet","zzz","sad"] if w in history_text])

        caps = 0
        if history_text:
            caps = sum(1 for c in history_text if c.isupper()) / max(len(history_text), 1)

        # Check recent chill (last 10 messages)
        recent_history = history_list[-10:] if history_list else []
        recent_chill = sum(1 for m in recent_history if any(w in m["content"].lower() for w in ["tired","cozy","zzz","soft"]))

        base = 0.5 + 0.15*hype - 0.18*chill + 0.20*caps
        if recent_chill >= 6 and "raid" in user_msg.lower():
            base = min(base, 0.65)
