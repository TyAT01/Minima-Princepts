...
class PersonaManager:
    ...
    def __init__(self, sheet_path: str = "fenilux_sheet.yaml"):
    ...
    def load_persona(self):
        ...
        search_paths: List[Path] = [
            self.sheet_path,
            Path("fenilux-ai/fenilux_sheet.yaml"),
            Path("../fenilux-ai/fenilux_sheet.yaml"),
            Path("fenilux_sheet.yaml"),
            # Search from script location
            Path(__file__).resolve().parent.parent / "fenilux_sheet.yaml",
            Path(__file__).resolve().parent.parent.parent / "fenilux-ai" / "fenilux_sheet.yaml",
        ]

        # Add even more candidate folders by looking for any folder named *chroma* or *Fenilux*
        try:
            cwd = Path.cwd()
            for p in [cwd, cwd.parent]:
                for candidate in p.glob("**/fenilux_sheet.yaml"):
        ...
        if not found_path:
            logger.error(f"Character sheet 'fenilux_sheet.yaml' not found. Please ensure it exists.")
            self.system_prompt = "You are Fenilux, a helpful AI companion."
            return
